...
let _aucon = class {
	...
	start_at(seek,newspeed,) {
		...
		this.audioData.forEach(({ src, when, offset, duration, volume, loop, myspeed = 1,}) => {
			//const { source, gainNode } = this.createSource(src, volume, loop,newspeed,); ASLI
			const { source, gainNode } = this.createSource(src, volume, loop,newspeed*myspeed,); //COBA

...
*/
let getsuara = null	;export let set_getsuara = a=>getsuara = a
let freecam = null	;export let set_freecam = a=>freecam = a
let setspf = null	;export let set_setspf = a=>setspf = a
...
let SHhelp	= que('#SHhelp'	)[0]; SHhelp	.addEventListener('click',fSHHelp	,)
let SHcontroller	= que('#SHcontroller'	)[0]; SHcontroller	.addEventListener('click',fSH	,)
let SHinfo	= que('#SHinfo'	)[0]; SHinfo	.addEventListener('click',fSH	,)
...
let fps = e=>{
	let ele = que('#fps'	)[0]
	let stepcount = +attr(ele,'stepcount',)
	stepcount = (stepcount < 6)?(stepcount+1):1
	attr(ele,'stepcount',stepcount,)
	setspf(stepcount)
	ele.firstElementChild.textContent = 60/stepcount
}
let seekreset = e=>(!(e.detail % 2)) && getsuara().setCurTime(0)
let speedreset = e=>(!(e.detail % 2)) && getsuara().setspeed(1)
...
export let contentloaded = a=>SHinfo.classList.contains('nyala') && fSH({currentTarget:SHinfo,})
...
que('#fps'	)[0].addEventListener('click'	,fps	,)
que('#showseek'	)[0].addEventListener('click'	,seekreset	,);		que('#showseek')[0].textContent = '----'
que('#showspeed'	)[0].addEventListener('click'	,speedreset	,);		que('#showspeed')[0].textContent = '----'
...
`Selamat datang di Poly Landhep v4.3, silakan -->> DOUBLECLICK <<-- info ini (expand & collapse)

--------------------------
sementara tidak ada info
